...
<base href="${basepath}">
	...
	<body>
		<div class="wrapper">
						...
						<div class="seach seach1 clearfix">
						    状态：<button class="task-btn task-btn-l task-btn-active" onclick="db_seach()" >待办</button><button class="task-btn task-btn-r" onclick="yb_seach()"> 已办</button> 
                      ...
                      <!--   <div class="panel-body">
							...
							<div class="seach seach2 clearfix">
								<button class="lf-btn lf-seach-screening lf-marginleft10px seach-screening">启用筛查</button>
								<button class="lf-btn lf-seach-btn margin0 seach-btn" onclick="seachrevalinitsTask()">查询</button>
								<input type="text" id="revalitask_title"  placeholder="搜索任务名称" autocomplete="off" class="lf-input lf-seach-inp seach-inpt">
							    高级搜索开始
								<div class="filter-select hide2 clearfix">
									<div class="filter-wrap left">
										<form class="form-horizontal m-t clearfix" id="real_task_condition_senior" action="">								
										</form>
									</div>
								</div>
								高级搜索结束
	...
	<iframe id="exportForm"    style="display:none;"></iframe>
		<!--审核结束-->
		<script src="js/b_module/taskReceive/wjsd.js" type="text/javascript" charset="utf-8"></script>
	    <script>
